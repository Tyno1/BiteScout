...
} from "shared/types/restaurant/delete";
import type {
...
} from "shared/types/restaurant/update";
...
type RestaurantListGetApiResponse = GetRestaurantResponse[] | ApiError;
type RestaurantDetailGetApiResponse = GetRestaurantResponse | ApiError;
type RestaurantDetailPutApiResponse = UpdateRestaurantResponse | ApiError;
type RestaurantDetailDeleteApiResponse = DeleteRestaurantResponse | ApiError;
...
export const filterRestaurants = async (
	...
	try {
		...
		const restaurants = await RestaurantData.find(filter)
			.populate("cuisine")
			.populate("gallery")
			.populate("assignedImages.logo.mediaId")
			.populate("assignedImages.logo.assignedBy")
			.populate("assignedImages.profileImage.mediaId")
			.populate("assignedImages.profileImage.assignedBy")
			.skip(skip)
			.limit(Number(limit))
			.sort({ name: 1 });
 ...
 */
async function getAvailableFilters(currentFilter: Record<string, unknown>) {
	...
	const cuisines = Array.from(
		new Set<string>(
			matchingRestaurants.flatMap(
				(r) => r.cuisine?.map((c: unknown) => String(c)) || [],
			),
		),
	);
	const priceRanges = Array.from(
		new Set<string>(
			matchingRestaurants.map((r) => r.priceRange).filter(Boolean),
		),
	);
	const features = Array.from(
		new Set<string>(matchingRestaurants.flatMap((r) => r.features || [])),
	);
...
export const getRestaurantById = async (
	req: Request<GetRestaurantRequest>,
...
export const updateRestaurantData = async (
	req: Request<{ id: string }, unknown, UpdateRestaurantRequest>,
...
/**
 * Updates assigned images for a restaurant (logo and profile image)
 */
export const updateAssignedImages = async (
	req: Request<
		{ id: string },
		unknown,
		{ logo?: string; profileImage?: string }
	>,
	res: Response,
	next: NextFunction,
): Promise<void> => {
	try {
		const { id } = req.params;
		const { logo, profileImage } = req.body;
		...
		// Validate that the media IDs exist and belong to this restaurant's gallery
		const restaurant = await RestaurantData.findById(id);
		if (!restaurant) {
			return next(createError(ErrorCodes.NOT_FOUND, "Restaurant not found"));
		}
		...
		if (logo !== undefined) {
			if (logo === null) {
				...
				updateData["assignedImages.logo"] = null;
			} else {
				// Validate that the logo media ID exists in the restaurant's gallery
				if (!restaurant.gallery.includes(logo)) {
					return next(
						createError(
							ErrorCodes.BAD_REQUEST,
							"Logo media must be from the restaurant's gallery",
						),
					);
				}
				updateData["assignedImages.logo"] = {
					mediaId: logo,
					assignedAt: new Date(),
		...
		if (profileImage !== undefined) {
			if (profileImage === null) {
				// Remove profile image assignment
				updateData["assignedImages.profileImage"] = null;
			} else {
				// Validate that the profile image media ID exists in the restaurant's gallery
				if (!restaurant.gallery.includes(profileImage)) {
					return next(
						createError(
							ErrorCodes.BAD_REQUEST,
							"Profile image media must be from the restaurant's gallery",
						),
					);
				}
				updateData["assignedImages.profileImage"] = {
					mediaId: profileImage,
					assignedAt: new Date(),
					assignedBy: req.user?.userId,
				};
			}
...
export const deletedRestaurant = async (
	req: Request<DeleteRestaurantRequest>,
...
export const getDeliveryLinks = async (
	...
	try {
		...
		const restaurant =
			await RestaurantData.findById(id).select("deliveryLinks");
