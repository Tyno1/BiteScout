...
export const searchFoodCatalogue = async (
	...
	try {
		...
		if (cuisine && typeof cuisine === "string") {
			...
			const cuisineDoc = await CuisineType.findOne({
				name: new RegExp(cuisine, "i"),
			});
			if (cuisineDoc) {
		...
		if (priceRange && typeof priceRange === "string") {
			...
			if (priceRanges[priceRange as keyof typeof priceRanges]) {
				filter["price.amount"] =
					priceRanges[priceRange as keyof typeof priceRanges];
			}
		}
...
async function getAvailableFilters(currentFilter: Record<string, any>) {
	try {
		...
		const cuisines = [
			...new Set(foods.map((f) => f.cuisineType?.name).filter(Boolean)),
		];
		const courses = [
			...new Set(foods.map((f) => f.course?.name).filter(Boolean)),
		];
		const priceRanges = [
			...new Set(foods.map((f) => f.priceRange).filter(Boolean)),
		];
		const allergens = [
			...new Set(
				foods
					.flatMap((f) => f.allergens?.map((a: any) => a.name) || [])
					.filter(Boolean),
			),
		];
