...
import { useUpdateUserProfile } from "@/hooks/user-profile/mutations/useUserProfileMutations";
...
export function UserEditModal({
	...
	// Determine if this is the current user editing their own profile
	const isCurrentUser = session?.user?._id === user._id;
	const isUpdating = isCurrentUser ? isUpdatingProfile : isUpdatingUser;
	...
	const handleSave = useCallback(() => {
		if (user._id) {
			...
			if (isCurrentUser) {
				// Use profile mutation for current user (includes session update)
				updateUserProfile(
					{
						userId: user._id,
						data: formData,
