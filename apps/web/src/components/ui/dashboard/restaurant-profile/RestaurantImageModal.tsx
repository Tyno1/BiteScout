...
export const RestaurantImageModal: React.FC<RestaurantImageModalProps> = ({
  ...
  const handleLogoChange = useCallback((mediaId: string | null) => {
    updateLogo(mediaId);
  }, [updateLogo]);

  const handleProfileImageChange = useCallback((mediaId: string | null) => {
  ...
  const UploadTab = useCallback(() => {
    return (
      <div className="space-y-6">
        <div>
          <h3 className="text-lg font-medium text-card-foreground mb-4">Upload New Media</h3>
          <p className="text-sm text-muted-foreground mb-4">
            Upload new images to your gallery. They will be automatically added to your restaurant's media collection and can then be assigned as logo or profile image.
          </p>
          <MediaUpload
              uploadMode="manual"
              uploadedFiles={[]}
  ...
  const GalleryTab = useCallback(() => {
    return (
      <div className="space-y-6">
        <div>
          <h3 className="text-lg font-medium text-card-foreground mb-4">Select from Gallery</h3>
          <p className="text-sm text-muted-foreground mb-4">
            Choose images from your existing gallery to assign as logo or profile image.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              {assignedImages?.logo?.mediaId?.url ? (
                <div className="relative">
                  ...
                  <div className="mt-2 text-sm text-muted-foreground">
                    <p>Assigned: {assignedImages.logo.assignedAt ? new Date(assignedImages.logo.assignedAt).toLocaleDateString() : 'Unknown'}</p>
                    <p>By: {assignedImages.logo.assignedBy?.name || 'Unknown'}</p>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <ImageIcon size={20} className="text-muted-foreground" />
                <h4 className="font-medium text-card-foreground">Profile Image</h4>
              </div>
              ...
              {assignedImages?.profileImage?.mediaId?.url ? (
                <div className="relative">
                  ...
                  />
                  <div className="mt-2 text-sm text-muted-foreground">
                    <p>Assigned: {assignedImages.profileImage.assignedAt ? new Date(assignedImages.profileImage.assignedAt).toLocaleDateString() : 'Unknown'}</p>
                    <p>By: {assignedImages.profileImage.assignedBy?.name || 'Unknown'}</p>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
          ...
          {/* Gallery Selection */}
          <div>
            <h4 className="text-lg font-medium text-card-foreground mb-3">Select Images to Assign</h4>
            <p className="text-sm text-muted-foreground mb-4">
              Click on any image below to assign it as your logo or profile image. Newly uploaded images will appear here automatically.
            </p>
            {isLoading && (
              <Alert 
                status="information" 
              ...
              className="mb-4"
            >
              Quick Tip: Click on any image below to assign it as your profile image. This image will be displayed in the hero section of your restaurant profile.
            </Alert>
            <Gallery
...
}, [assignedImages, gallery, handleLogoChange, handleProfileImageChange, restaurantId, isLoading]);
