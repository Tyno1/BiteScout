...
export const SkeletonPresets = {
  // Text skeleton
  Text: (props: Partial<SkeletonProps>) => <Skeleton height="sm" width="full" {...props} />,

  // Title skeleton
  Title: (props: Partial<SkeletonProps>) => <Skeleton height="md" width="3/4" {...props} />,
  ...
  List: ({ count = 5, ...props }: Partial<SkeletonProps> & { count?: number }) => (
    <div className="space-y-3">
      {Array.from({ length: count }, (_, index) => (
        <Skeleton
          key={`skeleton-list-${index}-${Date.now()}`}
          height="md"
          width="full"
          {...props}
        />
