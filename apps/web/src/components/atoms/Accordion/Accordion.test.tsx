...
describe("Accordion", () => {
	test("should render", () => {
		...
		expect(
			screen.getByRole("button", { name: "Accordion Item 1" }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("button", { name: "Accordion Item 2" }),
		).toBeInTheDocument();
	});
	...
	test("should open item when clicked", async () => {
		...
		expect(
			await screen.findByText("Accordion Item 1 Content"),
		).toBeInTheDocument();
	});
	...
	test("should close item when clicked", async () => {
		...
		expect(
			screen.queryByText("Accordion Item 1 Content"),
