...
describe("Input", () => {
	...
	test("uses custom id when provided", () => {
		render(<Input {...defaultProps} id="custom-id" useLabel />);
		const input = screen.getByRole("textbox");
	...
	test("applies correct outline types", () => {
		const { rerender } = render(
			<Input {...defaultProps} outlineType="round" />,
		);
	...
	test("generates unique id for different labels", () => {
		const { rerender } = render(
			<Input label="First Input" name="first" type="text" useLabel />,
		);
	...
	test("handles special characters in label for id generation", () => {
		render(
			<Input
				label="Input with Spaces & Special!"
				name="special"
				type="text"
				useLabel
			/>,
		...
		expect(screen.getByRole("textbox")).toHaveAttribute(
			"id",
			"input-input-with-spaces-&-special!",
		);
	});
});
