...
describe("Textarea", () => {
  ...
  test("applies correct outline types", () => {
    const { rerender } = render(<Textarea {...defaultProps} outlineType="round" />);
  ...
  test("applies correct sizes", () => {
    ...
    expect(screen.getByRole("textbox")).toHaveClass("text-base", "px-4", "py-4");
  ...
  test("applies custom right button style", () => {
    const rightButton = <div data-testid="right-button">Click</div>;
    render(
      <Textarea {...defaultProps} rightButton={rightButton} rightButtonStyle="text-blue-500" />
    );
  ...
  test("applies correct ARIA attributes", () => {
    render(<Textarea {...defaultProps} useLabel errorMessage="Error" helperText="Helper" />);
    const textarea = screen.getByRole("textbox");
  ...
  test("applies right button aria label", () => {
    ...
    expect(button).toHaveAttribute("aria-label", "Action button for Test Textarea");
  });
  ...
  test("generates unique id for different labels", () => {
    const { rerender } = render(<Textarea label="First Textarea" name="first" useLabel />);
    expect(screen.getByRole("textbox")).toHaveAttribute("id", "textarea-first-textarea");

    rerender(<Textarea label="Second Textarea" name="second" useLabel />);
    expect(screen.getByRole("textbox")).toHaveAttribute("id", "textarea-second-textarea");
  });

  test("handles special characters in label for id generation", () => {
    render(<Textarea label="Textarea with Spaces & Special!" name="special" useLabel />);
