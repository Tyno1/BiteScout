...
describe("Select", () => {
	...
	test("uses custom id when provided", () => {
		render(<Select {...defaultProps} id="custom-id" useLabel />);
		const select = screen.getByRole("combobox");
	...
	test("applies correct outline types", () => {
		const { rerender } = render(
			<Select {...defaultProps} outlineType="round" />,
		);
	...
	test("applies right button aria label", () => {
		...
		expect(button).toHaveAttribute(
			"aria-label",
			"Action button for Test Select",
		);
	});

	test("generates unique id for different labels", () => {
		const { rerender } = render(
			<Select label="First Select" name="first" options={[]} useLabel />,
		);
		...
		rerender(
			<Select label="Second Select" name="second" options={[]} useLabel />,
		);
		expect(screen.getByRole("combobox")).toHaveAttribute(
			"id",
	...
	test("handles special characters in label for id generation", () => {
		render(
			<Select
				label="Select with Spaces & Special!"
				name="special"
				options={[]}
				useLabel
			/>,
